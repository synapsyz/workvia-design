...
const seedGroups: Group[] = [
  ...
  },
  {
    id: 2,
    name: "Design",
    description: "Design system & visuals",
    color: "indigo",
    memberIds: [2, 3],
  },
];
...
const brand = "#0A236E"; // keep your brand color
        ...
        {editingUser && (
          <Modal onClose={() => setEditingUser(null)} title="Edit User">
            <div className="grid grid-cols-1 gap-3">
              ...
              <Select
                label="Manager"
                value={String(editingUser.managerId ?? "")}
                onChange={(e) =>
                  setEditingUser({
                    ...editingUser,
      ...
      <AnimatePresence>
        {showCreateGroupModal && (
          <Modal onClose={() => setShowCreateGroupModal(false)} title="Create Group">
            <div className="grid grid-cols-1 gap-3">
              ...
              <Select
                ...
                onChange={(e) =>
                  setGroupForm((p) => ({
                    ...p,
                    color: e.target.value as Group["color"],
                  }))
                }
                options={[
              ...
              <MultiSelect
                ...
                values={groupForm.memberIds}
                onToggle={(id) =>
                  setGroupForm((p) => {
                    const exists = p.memberIds.includes(id);
                    return {
                      ...p,
                      memberIds: exists
            ...
            <div className="mt-5 flex justify-end gap-2">
              <Button variant="ghost" onClick={() => setShowCreateGroupModal(false)}>
                Cancel
              </Button>
      ...
      <AnimatePresence>
        {deletingGroup && (
          <Modal onClose={() => setDeletingGroup(null)} title="Delete Group">
            <div className="text-sm text-gray-700">
              Are you sure you want to delete{" "}
              <span className="font-medium">{deletingGroup.name}</span>?
            </div>
            <div className="mt-5 flex justify-end gap-2">
...
function Button({
  ...
  return (
    <button onClick={onClick} className={`${base} ${styles}`} style={{ ["--brand" as any]: brand }}>
      {children}
    </button>
  );
}
